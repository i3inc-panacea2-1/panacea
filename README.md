...
The changed in Panacea v2.1 are:
* The `Core` will be responsible for managing plugins **only** (load/unload, manage errors etc). The `Core` package will be updated only when a new feature related to plugins is added (never happened so far) or if a bug related to this small functionality has been solved.
